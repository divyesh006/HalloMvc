...
namespace HalloDoc_mvc.Controllers
{
...

    {
        ...
        public IActionResult FamilyForm(FamilyForm model)
        {
            ...
            {
                /*                var user = _context.AspNetUsers.FirstOrDefault(u => u.Email == model.FamilyEmail);
                                if (user == null)
                                {
                                    var subject = "Create Account";
                                    var message = "Test";

                                    _emailSender.SendEmailAsync("pdivy2653@gmail.com", subject, message); 
                                }*/
                return RedirectToAction("Index", "Home");
            }
            return View();
        ...
        [HttpPost]
        public IActionResult FileUpload(FileUplaod model)
        {
        ...
        public IActionResult PatientDashboard()
        {
            ...
            List<Request> userData = _context.Requests.Where(u => u.UserId.ToString() == userEmail).ToList();
            var user = _context.Users.FirstOrDefault(u => u.UserId.ToString() == userEmail);
            ViewBag.RequestFile = _context.RequestWiseFiles.ToList();
            ViewBag.username = user.FirstName + " " + user.LastName;
            Dictionary<int, int> requestIdCounts = new Dictionary<int, int>();
            foreach (var request in userData)
            {
                int count = _context.RequestWiseFiles.Count(rwf => rwf.RequestId == request.RequestId);
        ...
        public IActionResult Update(UpdateProfile model)
        {
            ...
            if(user != null)
            {
                ...
                {                                                         
                        ...
                        aspUser.PhoneNumber = model.Mobile;
